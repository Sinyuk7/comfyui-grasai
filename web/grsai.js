...
function resetRemoteState(node) {
...
function baseUrl(node) {
  return String(widget(connectedConfigNode(node), "base_url")?.value ?? "").trim() || catalog.base_url;
}
...
function resetConfigConsumers(configNode) {
  for (const linkId of configNode.outputs?.[0]?.links ?? []) {
    ...
    if (states.has(target)) resetRemoteState(target);
  }
}
...
async function checkModel(node, model) {
  ...
  try {
    const response = await api.fetchApi("/grsai/model-status", {
      method: "POST",
...
function decorateParameters(node) {
  ...
  for (const [name, rule] of Object.entries(profile.parameters)) {
    ...
    item.label = name === "aspectRatio" ? (profile.family === "nano_banana" ? "Aspect Ratio" : "Image Size")
      : name === "imageSize" ? "Resolution" : "Quality";
...
app.registerExtension({
  ...
  async setup() {
    ...
    for (const event of ["grsai.balance", "grsai.progress", "grsai.batch"]) {
      api.addEventListener(event, ({ detail }) => {
        ...
        const state = states.get(node);
        if (!state) return;
        if (!acceptEvent(state, detail, node.properties.grsai_ui_token)) return;
  ...
  nodeCreated(node) {
    ...
    if (node.comfyClass === "GRSAIAPIConfig") {
      for (const name of ["api_key", "base_url", "token"]) {
    ...
    if (directory) {
      ...
      node.onConnectionsChange = function (type, slot, connected, ...rest) {
        ...
        if (this.inputs?.[slot]?.name === "api_config") resetRemoteState(this);
        labelReferences();
        return result;
      };
    } else {

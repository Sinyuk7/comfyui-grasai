# ComfyUI GRSAI 图片生成节点设计

状态：第一版核心节点已实现并完成离线/目标 ComfyUI 集成验证；已授权完成 Nano Banana 2 与 GPT Image 2.5 各一次真实生成，详见 [真实测试记录](live-test-2026-09-16.md)。余额 API 尚未真实验证。
...
生成流程接入 `POST /v1/api/generate` 与 `GET /v1/api/result?id=...`。两类模型共享提交路径，参数不同。另接入 `POST /client/openapi/getAPIKeyCredits` 用于节点内余额显示，生成结束后自动刷新。
...
| 图片输入 | 单个可选 `images` 插口，通过连线输入，支持批次与列表 |
| 多图含义 | 有序参考图，共同参与一次请求，不逐图生成 |
| API Key | 普通输入框，允许随工作流保存；不是安全密钥存储功能 |
...
| 余额 | 同一节点内显示当前 Key 的剩余积分，每次生成结束后自动刷新；查询不影响图片结果 |

此前“合并尺寸选项”“相同参数命中缓存”“执行中禁用按钮”的方案已被本表取代。

## 3. 节点界面

显示名称建议 `GRSAI Image`；稳定节点 ID 建议 `GRSAIImageGenerate`，避免今后修改名称破坏工作流。

固定输入：`api_key` 单行字符串、`model` 下拉、`prompt` 多行字符串、`images` 可选 IMAGE 连线。提示词支持由上游字符串连接；一个生成请求只能有一个提示词。

另有只读状态“剩余积分”：初始为未查询，生成结束后异步更新。它不是工作流输入或输出，不新增余额节点或生成按钮。详细规则见第 10 节。
...
JPG/JPEG/PNG/WEBP 文件由上游加载节点解码，本节点接收张量而非文件路径；内部转为 PNG 字节并 base64 编码。原文件格式、EXIF、ICC 和 alpha 是否保留取决于上游解码，不承诺保持原文件二进制或元数据。不连接图片则发 `images: []`。
...
balance.py                  独立的 API Key 余额查询与解析
...
参考：[接口笔记](api-notes.md)、[参考项目核查](reference-project-review.md)、[资料索引](references/README.md)。

## 10. API Key 积分余额

用户已选择：同一生成节点显示剩余积分，每次生成结束后自动刷新。无需额外 token、余额节点或手动刷新按钮；直接使用本次执行的 API Key。

契约依据：[用户提供的原文与域名核查](references/user-provided-api-key-credits.md)。请求地址为当前配置的主机根路径加 `/client/openapi/getAPIKeyCredits`，不能误拼为 `/v1/client/...`。使用 `Content-Type: application/json`，请求体为 `{"apiKey":"..."}`；不从生成接口推导额外 Bearer 认证要求。
...
界面显示“剩余积分：10,000”和最近查询时间，不换算人民币/美元，不将前后余额差当作当前图片的精确费用。该接口只提供余额，不能证明单次扣费；其他任务也可能同时消耗同一 Key。
...
- Key 或主机改变立即清除旧显示；原查询晚到的响应不得覆盖新 Key 的状态。相同 Key 的多次执行结果也应按查询序号避免旧响应覆盖新响应。
...
余额不写回模型配置、不持久化为可复用工作流结果。日志不打印包含 apiKey 的请求体，错误消息中出现 Key 时必须脱敏。刷新显示不应修改会影响执行的节点输入，不应自动把工作流重新加入队列。
